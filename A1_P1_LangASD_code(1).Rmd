...
```{r}
setwd("~/Dokumenty/3rd Semester/Experimental Methods III/Portfolio/01 Language development in ASD/Assignment 1")
```
...
```{r}
library(plyr)
demo=plyr::rename(demo, c("Child.ID"="ID" , "Visit"="VISIT"))
...
token$VISIT=as.numeric(token$VISIT)
```
...
```{r}
LU$ID=gsub("\\.","",LU$ID)
demo$ID=gsub("\\.","",demo$ID)
token$ID=gsub("\\.","",token$ID)
```
...
library(tidyverse)
demo_subset=select(demo,"ID","VISIT","Ethnicity", "Diagnosis","Gender", "Age", "ADOS", "nonVerbalIQ", "verbalIQ")
LU_subset = select(LU, 'ID','VISIT','MOT_MLU', 'MOT_LUstd', 'CHI_MLU', 'CHI_LUstd')
token_subset = select(token, 'ID', 'VISIT','types_MOT', 'types_CHI', 'tokens_MOT', 'tokens_CHI')
```
...
```{r}
demo_LU=merge(demo_subset,LU_subset)
data_final = merge(demo_LU,token_subset)
```
...
```{r}
subset_df=subset(data_final,VISIT==1,select=c('ID','ADOS', 'nonVerbalIQ','verbalIQ' ))
final = merge(subset_df,data_final,by="ID")
final_data = select(final,"ID","VISIT","Ethnicity", "Diagnosis","Gender", "Age", "ADOS.x", "nonVerbalIQ.x", "verbalIQ.x",'MOT_MLU', 'MOT_LUstd', 'CHI_MLU', 'CHI_LUstd','types_MOT', 'types_CHI', 'tokens_MOT', 'tokens_CHI')
...
```{r}
#check that 'VISIT' is numeric and not string
final_data$VISIT=as.numeric(final_data$VISIT)

#anonymize ID to numbers
library(dplyr)
str(final_data$ID) #find out what's ID
final_data$ID = final_data$ID %>% as.factor() %>% as.numeric()


#rename levels of gender and diagnosis
final_data$Gender=as.factor(final_data$Gender)
final_data$Gender = revalue(final_data$Gender,c('1'='F','2'='M'))

final_data$Diagnosis=as.factor(final_data$Diagnosis)
final_data$Diagnosis = revalue(final_data$Diagnosis,c('A'='ASD','B'='TD'))

#rename some variables
final_data = plyr::rename(final_data, c("ADOS.x" = "ADOS", "nonVerbalIQ.x" = "nonVerbalIQ", "verbalIQ.x" = "verbalIQ"))
```
...
```{r}
#load library for using pipes
library(magrittr)
#1.
more = filter(final_data, CHI_MLU > 2.7)
unique(more$ID)
...
```{r}
#1.
final_data %>% filter(VISIT==6)  %>% arrange(desc(tokens_CHI))

#2.
final_data %>% filter(VISIT==1)  %>% arrange(tokens_CHI)
```
...
subset_ASD=
  ...
  select(ID,CHI_MLU,MOT_MLU,tokens_CHI,tokens_MOT)

#2.
final_data %>% 
  filter(Diagnosis=="ASD") %>% 
  select(ID,CHI_MLU,MOT_MLU,tokens_CHI,tokens_MOT,ID, ID) #nothing happens ???
```
...
```{r}
#1.
final_data = mutate(final_data, GrandAvg = mean(tokens_CHI))

#2.
ChiAvg_AcrossVisits = final_data %>%
  group_by(ID)%>%
